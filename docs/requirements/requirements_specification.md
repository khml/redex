...
1. パーサ/評価
   ...
   - トークンは基本的に空白（スペース）で区切られる。ただし、括弧や演算子は空白なしでも認識される。
   - 文の文法（例）:
     - 定数定義: `const <IDENT> = <EXPR>`
     - 変数定義: `let <IDENT> = <EXPR>`
     - 式: `<TERM> ( ("+"|"-"|"*"|"/") <TERM> )*`
     - TERM: 数値リテラル（整数/浮動小数点）、識別子、または括弧で囲まれた式
   - 演算子の優先度は標準の算術優先度（`*`/`/` > `+`/`-`）。左結合。
   - 括弧 `( )` をサポートし、演算順序を明示的に制御できる。

2. 変数/定数のスコープ
   - 単一グローバルスコープを採用。
   - `const`で定義された名前は再代入不可（再代入を試みると `Redex::EvaluationError` を発生）。`let`は再代入可能。
